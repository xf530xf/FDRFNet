...
def parser():
    ...
    parser.add_argument('--savepath', default="/data3/YG/FDRFNet/code/model/R2C7K", type=str)
    parser.add_argument('--datapath', default="/data3/YG/FDRFNet/code/data/R2C7K/train", type=str)
...
def main(tr_cfg, te_cfg, tr_dl, te_dl, tr_sampler, net, args):
    ...
    for epoch in range(tr_cfg.epoch):
        ...
        with torch.no_grad():
            ...
            if args.rank == 0:
                with open("/data3/YG/FDRFNet/code/log/R2C7K/output.txt", "a") as f:
...
if __name__ == '__main__': 
    ...
    te_cfg = dataset.Config(datapath='/data3/YG/FDRFNet/code/data/R2C7K/test', mode='test')
    ...
    model = FDRFNet(tr_cfg).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    print(f'{total_params:,} total parameters.')



    checkpoint_path = checkpoint_path = os.path.join('/data3/YG/FDRFNet/code/model/tmp', "initial_weights.pt")
    if args.rank == 0:
        torch.save(model.state_dict(), checkpoint_path)
    dist.barrier()
    # checkpoint_path = '/data3/YG/FDRFNet/code/model/2/model-150'
    # dist.barrier()
