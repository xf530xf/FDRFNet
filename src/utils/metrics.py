...
class MAE(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        pred, gt = _prepare_data(pred, gt) # 归一化
...
class FbetaMeasure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        fbeta = self.cal_fbeta(pred, gt)
        # print(fbeta)
        self.fbeta_scores.append(fbeta)

    def cal_fbeta(self, pred: np.ndarray, gt: np.ndarray) -> float:
        if np.all(~gt):
            fbeta = 1
            return fbeta
    ...
    def get_results(self) -> dict:
        mean_fbeta = np.mean(np.array(self.fbeta_scores, dtype=_TYPE))
        return dict(mean_fbeta=mean_fbeta)
    
class AdaptiveFbetaMeasure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        # 计算自适应的β和Fβ
        fbeta = self.cal_adaptive_fbeta(pred, gt)
        # print(fbeta)
        self.fbeta_scores.append(fbeta)

    def cal_adaptive_fbeta(self, pred: np.ndarray, gt: np.ndarray) -> float:
        if np.all(~gt):
            fbeta = 1
            return fbeta
        pred = (pred > 0.5)
        ...
        # 动态调整 β
        adaptive_beta = recall / precision if precision != 0 else self.beta
        adaptive_beta = np.clip(adaptive_beta, 1e-10, 1e10)
        # 计算自适应的Fβ值
        if precision + recall == 0:
            fbeta = 0
        else:
            fbeta = (1 + adaptive_beta ** 2) * (precision * recall) / (adaptive_beta ** 2 * precision + recall)
        
        
        return fbeta

    def get_results(self) -> dict:
        ada_fbeta = np.mean(np.array(self.fbeta_scores, dtype=_TYPE))
        return dict(ada_fbeta=ada_fbeta)

class FmaxMeasure(object):
    def __init__(self, beta: float = 1):
        self.beta = beta
        self.fbeta_scores = []
    ...
    def cal_fbeta(self, pred: np.ndarray, gt: np.ndarray) -> float:
        ...
        pred = (pred > 0.5)  # 二值化预测结果

        tp = np.sum((pred == 1) & (gt == 1))  # True Positive
        fp = np.sum((pred == 1) & (gt == 0))  # False Positive
        fn = np.sum((pred == 0) & (gt == 1))  # False Negative

        precision = tp / (tp + fp) if (tp + fp) > 0 else 1  # 计算精确率
        recall = tp / (tp + fn) if (tp + fn) > 0 else 1  # 计算召回率

        if precision + recall == 0:
            fbeta = 0
        else:
            fbeta = (1 + self.beta**2) * (precision * recall) / (self.beta**2 * precision + recall)  # 计算 Fbeta

        return fbeta

    def get_results(self) -> dict:
        if not self.fbeta_scores:
            return dict(max_fbeta=0.0)  # 如果没有计算任何 Fbeta，则返回 0.0

        max_fbeta = np.max(np.array(self.fbeta_scores, dtype=_TYPE))
        return dict(max_fbeta=max_fbeta)

class EmeanMeasure(object):
    def __init__(self):
        self.adaptive_ems = []
        self.changeable_ems = []
...
class EmaxMeasure(object):
    ...
    def get_results(self) -> dict:
        adaptive_em = np.max(np.array(self.adaptive_ems, dtype=_TYPE))
        changeable_em = np.max(np.array(self.changeable_ems, dtype=_TYPE), axis=0)
        return dict(em=dict(adp=adaptive_em, curve=changeable_em))

if __name__ == '__main__':
    # gt_father_path = '/data3/YG/FRINet/code/data/COD10K/TestDataset/CAMO/mask'
    # pred_father_path = '/data3/YG/FRINet/code/result/data3/YG/FRINet/code/model/model-80/CAMO'
    # img_list = os.listdir(gt_father_path)
    # print(len(img_list))
    ...
    #     gt = cv2.imread(os.path.join(gt_father_path, item), cv2.IMREAD_GRAYSCALE)
    #     pred = cv2.imread(os.path.join(pred_father_path, item), cv2.IMREAD_GRAYSCALE)
    #     print(gt.shape)
    ...
    # print(sm1, adpem1, wfm1, mae1) # 0.85 0.916 0.794 0.053
    print(len(os.listdir('/data3/YG/FRINet/code/data/COD10K/test/mask')))

# mean F-beta measurement
